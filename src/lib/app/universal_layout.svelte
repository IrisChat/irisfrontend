...
<main class="inline-block lg:h-full lg:w-full overflow-hidden">
	<div class="wrapping flex flex-col lg:h-full"> <!-- Removing the overflow visibility would cause some very serious layout issues. Do not touch, please. -->
		<div class="lg:flex lg:h-full overflow-hidden lg:w-full">
			{#if gutter}
				<div
			...
			{#if sidebar}
				<div
					class="sidebar lg:min-w-1/4 lg:max-w-1/4 lg:overflow-y-auto overflow-x-hidden lg:h-full lg:w-1/4"
				>
					<slot name="sidebar">SIDEBAR</slot>
				</div>
			{/if}
			{#if interaction_pane}
				<div class="interaction-pane lg:min-w-3/4 lg:max-w-3/4 lg:h-full lg:w-3/4 lg:overflow-hidden"> <!-- Hide both otherwise it becomes weird -->
			...
			{#if participant_panel}
				<div class="participant-panel lg:min-w-1/4 lg:max-w-1/4 lg:h-full lg:w-1/4">
					<slot name="participant_panel"><div
					...
					>
						<div class="placeholder_hint__content block">
							...
							<div class="subtitle text-base font-light">Select a chat to view who you're talking with.</div>
						</div>
					</div></slot>
				</div>
			{/if}
